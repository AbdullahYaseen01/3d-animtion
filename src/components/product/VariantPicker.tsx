...
export const VariantPicker = forwardRef<HTMLFieldSetElement, Props>(function VariantPicker(
  ...
  const selectedStock = size != null ? stockFor(product, buildSku(product.id, color.slug, size, width.code)) : null
    ...
    <div className="variant-picker">
      ...
      >
        <legend className="vp-legend vp-legend--row">
          <span>
            Size <span className="muted">(US men's)</span>
            {size != null && (
              <>
            ...
            return (
              <label key={s} className={`vp-size${out ? ' vp-size--out' : ''}`}>
                <input
                  ...
                  onChange={() => onSize(s)}
                />
                <span aria-hidden="true">{formatSize(s)}</span>
                <span className="visually-hidden">
                  US men's {formatSize(s)}
                  {out ? ', sold out' : ''}
                </span>
              </label>
            )
